...
- **caveman** token-reduction skill installed as an enabled Claude Code plugin (`caveman@caveman`, user scope — verify with `claude plugin list`). Slash commands `/caveman`, `/caveman-commit`, `/caveman-review`, `/caveman-stats`, `/caveman-compress` (and "caveman mode") activate only after a Claude Code **restart** (plugins load at session start). Default intensity is `lite` unless the user asks for a different level. Installer: `npx -y github:JuliusBrussee/caveman` (npm `@juliusbrussee/caveman-code`); pins remote fetches to a release tag; uninstall via `... -- --uninstall`.
