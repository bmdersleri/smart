...
import { listPlcs, updatePlc } from '../api/client'
...
function PlcRow({ plc, onEdit }: { plc: PlcEntry; onEdit: () => void }) {
  return (
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        {total > 0 && (
          <div className="flex items-center gap-2 text-sm">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-medium ${connected === total ? 'bg-green-900/30 text-green-400' : 'bg-yellow-900/30 text-yellow-400'}`}>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-gray-900 rounded-xl border border-gray-800">
          <p className="text-gray-400">Henüz PLC kaydı yok.</p>
          <p className="text-gray-500 text-sm mt-1">Catalog import sonrası PLC'ler burada listelenir.</p>
        </div>
      ) : (
            ...
            <tbody>
              {plcs.map((plc) =>
                editingName === plc.name ? (
                  ...
                  <PlcRow key={plc.name} plc={plc} onEdit={() => setEditingName(plc.name)} />
                )
              )}
            </tbody>
          </table>
      ...
      <p className="text-xs text-gray-600">
        IP değiştirildiğinde yeni bağlantı bir sonraki poller döngüsünde kurulur.
      </p>
    </div>
  )
}
