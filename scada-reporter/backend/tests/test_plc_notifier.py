...
async def test_dispatch_calls_enabled_channels(monkeypatch):
    ...
    async def _fake_webhook(p):
        calls.append("webhook")

    async def _fake_email(p):
        calls.append("email")

    monkeypatch.setattr(notifier, "_send_webhook", _fake_webhook)
    monkeypatch.setattr(notifier, "_send_email", _fake_email)
