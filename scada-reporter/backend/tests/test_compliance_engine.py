...
from app.services.compliance_engine import build_event_key, evaluate_permit
...
async def _seed_hybrid_without_lab(db_session):
    ...
    db_session.add_all([permit, sample_point, point, tag, lab_parameter])
    await db_session.flush()

    point.lab_sample_point_id = sample_point.id
    parameter = ComplianceParameter(
        permit_id=permit.id,
        discharge_point_id=point.id,
        parameter_name="NH3",
        source_type="hybrid",
        tag_id=tag.id,
        lab_parameter_id=lab_parameter.id,
    )
    db_session.add(parameter)
    await db_session.flush()
...
async def _seed_bad_quality(db_session):
...
async def test_bad_quality_creates_event(db_session):
    permit, _, _ = await _seed_bad_quality(db_session)
    start = _ts(2026, 6, 1)
    end = _ts(2026, 6, 2)
