...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("quality", sa.Integer(), nullable=False, server_default=sa.text("192")),
        sa.Column("timestamp", sa.DateTime(), nullable=False),
        sa.PrimaryKeyConstraint("tag_id", "timestamp"),
        sa.ForeignKeyConstraint(["tag_id"], ["tags.id"], name="fk_tag_readings_tag_id"),
    )
    op.create_index("idx_tag_readings_tag_ts", "tag_readings", ["tag_id", sa.text("timestamp DESC")])
