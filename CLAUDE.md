...
- **Lab → Grafana dashboard**: `POST /api/grafana/dashboards/from-lab` (feature-gated `grafana`, any authenticated user) generates a Grafana dashboard from a lab sample point — one time-series panel per parameter with min/max limit lines + a latest-values table, uid `sr-lab-{point_id}-{hash}`, overwrite-on-regenerate, querying the `v_lab_timeseries` view (PostgreSQL/TimescaleDB datasource). `DELETE /api/grafana/dashboards/{uid}` (admin-only) deletes a managed dashboard. Both endpoints validate uid against `^[A-Za-z0-9_-]+$`.
